...
def reward_from_feedback(
    ...
    *,
    ...
    """Map explicit, interaction, and optional creative-alignment signals to [0, 1]."""
    ...
    if clip_score is not None:
        if not np.isfinite(clip_score):
            raise ValueError("clip_score must be finite when supplied")
        clip = float(np.clip(clip_score, 0.0, 1.0))
        acceptance_rate = 0.0 if interactions == 0 else accept_count / interactions
        edit_depth = 0.0 if interactions == 0 else (edit_count + regenerate_count) / interactions
        return float(np.clip(0.35 * explicit + 0.25 * acceptance_rate - 0.15 * edit_depth + 0.25 * clip, 0.0, 1.0))
