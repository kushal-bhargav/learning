...
class AgencyConsoleService:
    ...
    def __init__(
        ...
        *,
        ...
        store_path = experience_store_path or os.getenv("GMGI_EXPERIENCE_STORE") or "experiments/experience_store.jsonl"
    ...
    def submit_feedback(
        ...
        *,
        ...
        clip_score = creative.get("clip_score")
        ...
        self.experience_store.append(
            Episode(
                ...
                context_fingerprint=context_fingerprint(relationship, occasion),
                agent_outputs=self._collect_outputs(session),
                human_actions=self._collect_actions(session),
                composite_reward=reward,
                clip_score=None if clip_score is None else float(clip_score),
                prompt_versions=self._current_prompt_versions(),
            )
        )
    ...
    def _run_stage(self, console: ConsoleSession, stage: str, overrides: dict[str, Any]) -> AgentOutput:
        ...
        context_fp = context_fingerprint(relationship, occasion)
        demo_agents = os.getenv("GMGI_USE_DEMO_AGENT_RESPONSES", "0") == "1"
        llm_opts = {"retriever": self.experience_retriever, "prompt_versions_dir": self.prompt_versions_dir}
        if stage == "recipient_profiling":
            agent = RecipientProfilingAgent(DemoStructuredLLM(self._recipient_response(fixture)) if demo_agents else None, **llm_opts)
            ...
            agent = RelationshipAnalysisAgent(DemoStructuredLLM(self._relationship_response(fixture)) if demo_agents else None, memory_graph=self._memory_graph(console), **llm_opts)
            config = {"relationship": relationship, "memories": fixture.get("memories", []), "occasion": occasion, "recipient_id": recipient["id"], "occasion_id": occasion["id"]}
        elif stage == "gift_intent_reasoning":
            agent = GiftIntentReasoningAgent(DemoStructuredLLM(self._intent_response(fixture, occasion)) if demo_agents else None, **llm_opts)
            config = {
            ...
            agent = MultiAgentPlanningAgent(DemoStructuredLLM(self._planning_response(fixture, intent)) if demo_agents else None, **llm_opts)
            config = {
            ...
            agent = RecommendationAgent(DemoStructuredLLM(self._recommendation_response(fixture)) if demo_agents else None, bandit=self._load_existing_bandit(), **llm_opts)
            config = {
                "recipient_profile": self._safe_effective(console, "recipient_profiling"),
                "relationship_guidance": self._safe_effective(console, "relationship_analysis"),
                ...
                "occasion": occasion,
                "preferences": fixture.get("preferences", []),
                "bandit_context": self._bandit_context(relationship, occasion, console.agency_slider).tolist(),
            }
            ...
            visual_generation = intent.get("visual_generation", {}) if isinstance(intent, Mapping) else {}
            goal = intent.get("goal", {}) if isinstance(intent, Mapping) else {}
            gift_artifact_type = str(
                overrides.get("gift_artifact_type")
                or visual_generation.get("artifact_type")
                or goal.get("recommended_artifact_type")
                or "greeting_card"
            )
            ...
            config = {
                ...
                "diffusers_prompt": self._diffusers_prompt(relationship, occasion, memory, gift_artifact_type, visual_generation, agency_slider),
            }
        elif stage == "greeting_story":
            agent = GreetingStoryAgent(DemoStructuredLLM(self._greeting_response(fixture)) if demo_agents else None, **llm_opts)
            config = {
                "relationship_guidance": self._safe_effective(console, "relationship_analysis"),
                "occasion": occasion,
                "memories": fixture.get("memories", []),
            ...
            agent = DeliveryPlannerAgent(**llm_opts)
    ...
    @staticmethod
    def _finalize_agent_output(result: AgentOutput, agent: Any) -> AgentOutput:
        output = dict(result["output"])
        output.setdefault("prompt_version", str(getattr(agent, "prompt_version_id", "static")))
        config = getattr(agent, "config", {})
        if isinstance(config, Mapping):
            output.setdefault("skills_used", list(config.get("skills", [])))
        else:
            output.setdefault("skills_used", [])
        return AgentOutput(
            stage=result["stage"],
            output=output,
    ...
    def _memory_graph(self, console: ConsoleSession):
        if console.fixture_path is None:
            return None
        try:
            return load_fixture(console.fixture_path)
        except Exception:
            return None

    def _bandit_context(self, relationship: Mapping[str, Any], occasion: Mapping[str, Any], agency_slider: float) -> np.ndarray:
    ...
    def _load_existing_bandit(self) -> LinUCBBandit | None:
        if not self.bandit_state_path.exists():
            return None
        try:
            return LinUCBBandit.load(self.bandit_state_path)
        except Exception:
            return None

    def _collect_outputs(self, session: GiftSession) -> dict[str, Any]:
        outputs: dict[str, Any] = {}
        for entry in session.stage_log:
            if entry.status in {"pending", "completed"}:
                outputs[entry.stage] = self._jsonable_mapping(entry.output)
                if entry.human_action == HumanAction.EDIT and entry.human_edit:
                    outputs[entry.stage].update(self._jsonable_mapping(entry.human_edit))
        return outputs
    ...
    def _collect_actions(self, session: GiftSession) -> dict[str, str]:
    ...
    def _current_prompt_versions(self) -> dict[str, str]:
        ...
        for stage in STAGES:
            latest = self.prompt_versions_dir / stage / "latest.json"
            if latest.exists():
                try:
                    payload = json.loads(latest.read_text(encoding="utf-8"))
                    versions[stage] = str(payload.get("version_id") or latest.as_posix())
                except Exception:
                    versions[stage] = latest.as_posix()
            else:
                versions[stage] = "static"
        return versions
