...
def preprocess_images(records: list[dict[str, Any]], config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for record in records:
        if "source" not in record:
            record = {**record, "source": "art_institute_chicago" if str(record.get("id", "")).startswith("aic-") else "met_museum" if str(record.get("id", "")).startswith("met-") else "unknown"}
...
def write_data_card(records: list[dict[str, Any]], rejections: list[dict[str, Any]], report: dict[str, Any], config: dict[str, Any]) -> None:
    ...
    lines.extend([
        "",
        "The pipeline only retains records whose source metadata explicitly marks the work/image as public domain or open access under the configured source policy. Each manifest row stores the source object URL, image URL, license label, raw checksum, processed checksum, and rejection-independent provenance fields.",
        "",
        "Users remain responsible for non-copyright rights such as privacy, publicity, trademark, or culturally sensitive use.",
        "",
        "## Processing",
        "",
        "1. Query configured public/open-access museum APIs.",
        "2. Reject records without image URLs, public-domain/open-access flags, decodable image bytes, or configured minimum dimensions.",
    ...
    lines.extend([
        ...
        "## Limitations",
        "",
        "The emotion labels are weak CLIP-derived pseudo-labels, not human annotations. Museum collections are not a clean greeting-card corpus and may be uneven across time, geography, subject matter, and visual style. Scaling beyond a pilot run should include manual review, duplicate filtering, visual relevance filtering, and documented rejection reasons.",
        "",
